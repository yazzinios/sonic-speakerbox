...
import { ArrowLeft, Upload, Image, Music, Save } from 'lucide-react';
...
const SettingsPage = () => {
  ...
  const handleBgUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      localStorage.setItem('dj-bg', url);
      setBgPreview(url);
    };
  ...
  const handleJingleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      localStorage.setItem('dj-jingle-name', file.name);
  ...
  const clearBg = () => {
    localStorage.removeItem('dj-bg');
    setBgPreview('');
  };

  const resetJingle = () => {
    localStorage.removeItem('dj-jingle');
    localStorage.removeItem('dj-jingle-name');
    ...
    <div className="min-h-screen bg-background p-4 md:p-6">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <section className="rounded-lg border bg-card p-4 space-y-4">
          ...
          <p className="text-xs text-muted-foreground">Upload a custom jingle that plays before you go on air, or keep the default beep.</p>
          <div className="flex items-center gap-3">
            <input ref={jingleRef} type="file" accept="audio/*" className="hidden" onChange={handleJingleUpload} />
            <Button variant="outline" onClick={() => jingleRef.current?.click()}>
          ...
          {jingleName !== 'Default (tan-tan-tan)' && (
            <Button variant="ghost" size="sm" className="text-xs text-destructive" onClick={resetJingle}>
              Reset to default
            </Button>
          )}
        </section>
        ...
        {/* Background */}
        <section className="rounded-lg border bg-card p-4 space-y-4">
          <h2 className="text-sm font-bold tracking-wider text-foreground">BACKGROUND IMAGE</h2>
          <div className="flex items-center gap-3">
          ...
          {bgPreview && (
            <div className="space-y-2">
              ...
              <Button variant="ghost" size="sm" className="text-xs text-destructive" onClick={clearBg}>
                Remove background
              </Button>
            </div>
          )}
